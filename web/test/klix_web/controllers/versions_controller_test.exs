defmodule KlixWeb.VersionsControllerTest do
  ...
  @tag :tmp_dir
  test "can send multiple requests", %{conn: conn, tmp_dir: dir} do
    {:ok, image} =
      user_fixture()
    ...
    conn = put_versions(conn, image, datetime, signature)
    ...
    conn =
      conn
      |> recycle()
      |> put_versions(image, datetime, signature)
  ...
  defp put_versions(conn, image, datetime, signature) do
    conn
